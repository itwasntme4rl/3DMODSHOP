(function () {
  const stage = document.getElementById("plateCarrierViewer");
  const loading = document.getElementById("plateCarrierLoading");

  if (!stage || !window.THREE || !window.SIMPLE_PLATE_CARRIER_GLB_DATA_URI) {
    if (loading) {
  ...
  const fill = new THREE.DirectionalLight(0xc3c6bd, 1.1);
  ...
  function frameModel(object) {
    ...
    camera.position.set(0, maxSize * 0.08, distance * 1.8);
  ...
  loader.load(
    window.SIMPLE_PLATE_CARRIER_GLB_DATA_URI,
    function (gltf) {
